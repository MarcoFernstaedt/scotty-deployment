...
- `sudo /usr/local/sbin/scotty-start` is the root-only command that brings the deployment up. It is not the only lifecycle command: `sudo scotty-supervisor` owns releases and recovery — `publish`, `stage`, `accept`, `activate`, `uninstall`, backup, restore and rollback. Starting is one command; operating the deployment is that command plus the supervisor. It validates the prepared stopped container, runs owner-only setup, completes the runtime's own Codex OAuth, validates Google consent against the configured account and scopes, starts only the prepared container, runs non-sending checks, and on any partial failure leaves Scotty stopped with one recovery instruction. It never sends onboarding.

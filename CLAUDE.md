...
Scotty must be a normal, useful Discord assistant inside the configured client guild, not merely a setup wizard. Provision or reuse two separate private text channels with exact readback and idempotency: one bound to Trent's exact authorized user tuple and one bound to Mikey's exact authorized user tuple. Trent and Mikey must each be able to converse privately with Scotty, maintain separate session/memory context, receive replies and attachments, and run only the business operations allowed for their role. Neither client user may view, trigger, impersonate, inherit, or cross-route into the other's private channel or session. The bot and explicitly configured maintainer access may be present only as required for operation and support.

Within the configured client guild, expose typed Discord operations needed for ordinary assistant work: read configured channels, send/edit/delete Scotty's own messages, reply, react, attach approved files, create and manage task threads, and publish progress/status updates to explicitly configured shared destinations such as an announcements channel. Support near-real-time streamed task updates through rate-limited coalesced message edits or bounded progress posts so Scotty can report what it is doing without flooding the server. Every destination must be configured and read back; private channel content, credentials, maintainer details, and cross-user memory must never leak into announcements or another user's channel.

Routine reversible Discord work in the configured guild should remain low-friction. Consequence gates remain required for creating/deleting channels, changing roles or permissions, bans/kicks/moderation, webhooks or bot installation, destructive history cleanup, mass mentions, public exposure, new external audiences, bulk messaging, or any action outside configured guild/channel authority. Do not translate "do whatever they need in Discord" into Discord Administrator, arbitrary REST calls, permission escalation, or cross-guild access. Implement the broad useful surface through exact typed actions, actor/role binding, destination allowlists, idempotency, readback, audit receipts, and no blind retry after ambiguous sends.

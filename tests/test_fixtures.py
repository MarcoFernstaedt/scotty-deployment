...
class FixtureTests(unittest.TestCase):
    ...
    def test_discord_only_example_starts_with_no_connected_provider(self) -> None:
        raw = json.loads(
            (Path("fixtures") / "scotty.private.discord-only.example.json").read_text(
                encoding="utf-8"
            )
        )
        config = RuntimeConfig.from_mapping(raw)
        ...
        self.assertEqual(config.maintainer_route.profile, "scotty-maintainer")

    def test_every_fixture_identifier_is_a_synthetic_snowflake(self) -> None:
        raw = json.loads(
            (Path("fixtures") / "scotty.private.example.json").read_text(encoding="utf-8")
        )
        config = RuntimeConfig.from_mapping(raw)
        identifiers = [
            *(principal.guild_id for principal in config.principals),
